// modules dependencies
var path = require('path');
var webpack = require("webpack");

// plugins dependencies
var CleanWebpackPlugin = require('clean-webpack-plugin');
var ExtractTextPlugin = require('extract-text-webpack-plugin');
var HtmlWebpackPlugin = require('html-webpack-plugin');

//default behavior : we build a development environment
var dev = true;
var prod = false;
if (process.env.NODE_ENV === "prod") {
  dev = false;
  prod = true;
}
//for the dev/prod option, we stick to the CLI behavior, i.e. :
// dev (-d) = --debug --devtool source-map --output-pathinfo
// prod (-p) = --optimize-minimize --optimize-occurence-order (UglifyJsPlugin)
console.log(`env options: dev=${dev}, prod=${prod}`);

var webpackConfig = {
  /**BUILD STUFF**/
  target: 'web',
  cache: true,
  resolve: {
    extensions: ['', '.js', '.css'], //extensions to add when resolving modules (dont entry !)
    //directories in which we should look for modules
    root: [path.resolve(__dirname, 'src')], //root of our project (absolute path)
    modulesDirectories: ['node_modules', 'node_modules/bootstrap/dist/css'] //directories for modules resolved via node's resolving algorithm
  },
  context: path.resolve(__dirname, 'src'),
  entry: {
    // /!\ array: only the last module is exported !
    "secret-santa": ['vendor.js', 'app.js']
  },
  output: {
    filename: 'resources/[name]-bundle-[hash].js',
    path: path.join(__dirname, 'build'),
    libraryTarget: 'var',
    library: 'secretsanta',
    pathinfo: dev
  },
  module: {
    loaders: [{
      test: /\.css$/,
      loader: ExtractTextPlugin.extract('css-loader?sourceMap')
    }, {
      test: /\.eot$/,
      loader: "file-loader",
      query: {
        name: 'resources/[name].[ext]'
      }
    }, {
      test: /\.(woff|woff2)$/,
      loader: "url-loader",
      query: {
        limit: '5000',
        prefix: 'font/',
        name: 'resources/[name].[ext]'
      }
    }, {
      test: /\.ttf$/,
      loader: "url-loader",
      query: {
        limit: '10000',
        mimetype: 'application/octet-stream',
        name: 'resources/[name].[ext]'
      }
    }, {
      test: /\.svg$/,
      loader: "url-loader",
      query: {
        limit: '10000',
        mimetype: 'image/svg+xml',
        name: 'resources/[name].[ext]'
      }
    }, {
      test: /\.png$/,
      loader: "url-loader",
      query: {
        limit: '10000',
        mimetype: 'image/png',
        name: 'resources/[name]-[hash].[ext]'
      }
    }]
  },
  plugins: [
    new CleanWebpackPlugin(['build'], {
      root: path.resolve(__dirname),
      verbose: true,
      dry: false
    }),
    //cssExtractor
    new ExtractTextPlugin('resources/[name]-bundle-[hash].css'),
    //This plugins will (maybe not in that order..):
    //1- load the html page through the html-loader. This will change <img src="image.png"> into require("./image.png")
    //2- resolve the require of the images (using the specified file or url loaders)
    //3- add a link to our js bundle
    //4- add a link to our extracted css bundle
    //6- add a link to the favicon
    // TODO: hash favicon: passer par une favicon.ico et url-loader? + build time + virer dependances bootstrap ?
    new HtmlWebpackPlugin({
      template: 'html-loader!./src/index.html',
      favicon: 'images/favicon.png'
    }),
    new webpack.ProvidePlugin({
      $: 'jquery',
      jQuery: "jquery"
    })
  ],

  /**DEV STUFF**/
  debug: dev,
  devtool: dev ? "sourcemap" : false, //generate source map for js (css source map are generated by the css loader)
  devServer: {
    "port": 9090,
    "contentBase": "build",
    "inline": true //automatically reload page
  }
};

if (prod) {
  //this plugin will minify our scripts and our css (because we use the css loader)
  webpackConfig.plugins.push(new webpack.optimize.UglifyJsPlugin({}));
  //this plugin will vary the distribution of the ids to get the smallest id length for often used ids
  webpackConfig.plugins.push(new webpack.optimize.OccurenceOrderPlugin());
}

// TODO: corriger l'appli : faire une fonction "addEvent"

module.exports = webpackConfig;
